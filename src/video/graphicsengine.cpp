...
void GraphicsEngine::draw_texture(Texture* tex, Rect dest, Color color, Flip flip, float angle,
                                  Vector center)
{
	draw_texture(tex, {{0, 0}, tex->get_size()}, dest, color, flip, angle, center);
}

void GraphicsEngine::draw_texture(Texture* tex, Vector pos, Color color, Flip flip, float angle,
                                  Vector center)
{
	draw_texture(tex, {pos, tex->get_size()}, color, flip, angle, center);
}

void GraphicsEngine::draw_texture(Texture* tex, Rect_t<int> src, Rect dest, Color color, Flip flip,
                                  float angle)
{
	draw_texture(tex, src, dest, color, flip, angle, tex->get_size() / 2);
}

void GraphicsEngine::draw_texture(Texture* tex, Rect dest, Color color, Flip flip, float angle)
{
	draw_texture(tex, {{0, 0}, tex->get_size()}, dest, color, flip, angle, tex->get_size() / 2);
}

void GraphicsEngine::draw_texture(Texture* tex, Vector pos, Color color, Flip flip, float angle)
{
	draw_texture(tex, {pos, tex->get_size()}, color, flip, angle, tex->get_size() / 2);
}
